...
- 字幕：已归档 1 个分P，约 2.6 分钟，80 行字幕，1,039 字，6 个字幕证据块。
- 评论：已归档 18 条评论/回复，其中主评论 14 条，子回复 4 条，生成 18 个评论证据块；B站接口显示约 18 条。
- 元数据：已归档：metadata.json, source.md, run_summary.json, subtitle_probe.json；缺少：opus_raw.json, opus_normalized.json。
- 笔记预算：推荐笔记长度 1,448-2,100 字；互动质量倍率 1.207（播放 8,396，点赞 332，收藏 248，投币 44，评论 18，弹幕 1，分享 24；发布于 2026-04-08，距今 103 天）。以核心观点、证据和少量实践建议为主，避免过度扩写。
