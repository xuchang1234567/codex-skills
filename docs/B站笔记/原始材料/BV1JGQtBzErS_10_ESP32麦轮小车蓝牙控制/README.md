...
- 字幕：已归档 1 个分P，约 6.8 分钟，191 行字幕，2,308 字，14 个字幕证据块。
- 评论：已归档 17 条评论/回复，其中主评论 12 条，子回复 5 条，生成 17 个评论证据块；B站接口显示约 18 条。
- 元数据：已归档：metadata.json, source.md, run_summary.json, subtitle_probe.json；缺少：opus_raw.json, opus_normalized.json。
- 笔记预算：推荐笔记长度 1,453-2,107 字；互动质量倍率 1.211（播放 9,504，点赞 286，收藏 293，投币 40，评论 18，弹幕 2，分享 39；发布于 2026-04-13，距今 98 天）。以核心观点、证据和少量实践建议为主，避免过度扩写。
