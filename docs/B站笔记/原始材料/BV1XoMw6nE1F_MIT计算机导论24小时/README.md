...
- 字幕：已归档 29 个分P，约 1714.6 分钟，32,786 行字幕，290,115 字，2,354 个字幕证据块。
- 评论：已归档 4 条评论/回复，其中主评论 3 条，子回复 1 条，生成 4 个评论证据块；B站接口显示约 4 条。
- 元数据：已归档：metadata.json, source.md, run_summary.json, subtitle_probe.json；缺少：opus_raw.json, opus_normalized.json。
- 笔记预算：推荐笔记长度 57,600-65,000 字；互动质量倍率 1.280（播放 2,050，点赞 100，收藏 384，投币 18，评论 4，弹幕 15，分享 12；发布于 2026-07-05，距今 15 天）。按模块/分P写，保留逐P表；不要压成短视频式摘要。 如需详细拆解，补关键帧或视觉理解；若只基于字幕写作，必须明确覆盖范围。 画面依赖提示：字幕/转写文本偏少，适合先写有限摘要；如果用户要详细课程笔记，应补视觉证据或明确覆盖范围。
