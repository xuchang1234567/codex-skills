...
Bili Note 是一个面向知识库的 B 站视频与图文笔记工具：完整归档字幕、图文正文、图片与评论，把 B 站内容整理成可学习、可检索、可追问的 Markdown 笔记。默认知识库路径为 `C:\Users\dxc\Desktop\知识库\B站笔记`，可用 `--archive-dir` 覆盖。

它的核心特点是：

- 完整归档：保存完整字幕、图文正文、图片、完整评论、元数据和证据索引，主笔记中的关键判断可以通过论文式编号链接回到原文位置复核。
- 非固定长度：不把短视频和长课程压成同样字数；预算只提供写作参考和 QA，不设最长限制，证据充分时保留完整细节。
- 质量感知：结合内容热度、互动质量、评论讨论度和发布时间等信号调整笔记预算，让更值得深读的内容获得更充分的整理。
- 写前定标：先根据原始材料了解推荐字数、信息密度和写作粒度，再开始写笔记；写后评分只用于验收和发现遗漏，不自动截断长笔记。
...
默认保存到 `C:\Users\dxc\Desktop\知识库\B站笔记`。如果你想覆盖默认路径，再加一句保存路径：
...
- 写笔记前必须先读取 `metadata/note_budget.json`，把推荐字数区间、写作粒度、质量倍率和画面依赖提示当作参考；没有最长限制，写完后再用评分做验收，`too_long` 只提示检查重复而不删除有证据的内容。
