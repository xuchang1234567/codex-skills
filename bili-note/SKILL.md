...
Bili Note 默认知识库根目录绑定为 `C:\Users\dxc\Desktop\知识库\B站笔记`：主笔记放在根目录下，原始材料归档放在 `原始材料\<BVID或opus-id>`。显式提供 `--archive-dir` 时以显式路径为准；默认目录不存在时由归档流程创建。临时提取目录仍放在工作目录或 `tmp_bili_note_*` 下。
...
7. 归档原始材料：把完整字幕或图文正文、图片、完整评论、元数据和 JSONL 索引存到 `C:\Users\dxc\Desktop\知识库\B站笔记\原始材料` 下的长期目录。
8. 写前定标：必须先读取 `metadata/note_budget.json`，把推荐字数区间、写作粒度、互动质量倍率、证据块数量和 `visual_dependency` 作为本次笔记的参考。预算用于 QA 和发现遗漏，不是硬性截断上限；证据支持时应保留完整技术细节，宁可写长，不要为凑区间删减内容。
   - 如果 `visual_dependency.risk` 是 `medium` 或 `high`，先补关键帧/OCR/多模态视觉理解，或明确告诉用户当前缺少视觉理解能力，不能写成完整解析。
9. 按材料写 Markdown：默认写成“学习型笔记”，目标是让人或 Agent 像学完一节课或读完一篇教程一样获得概念、方法、判断标准、实践步骤和自测题；根据内容结构决定详略，不要把长课和短视频写成差不多字数，也不要设置最长字数或自动截断。来源、覆盖范围和归档路径放到后半部分。正文证据默认用论文式编号 `[1][2]`，不直接堆长证据 ID。
...
- `status`：`too_short` 表示遗漏风险高；`too_long` 仅提示检查明显重复或原文倾倒，不得自动删除、截断有证据支持的技术内容；`ok` 表示落在写前推荐区间。
- `quality_multiplier`：点赞、收藏、投币、评论、弹幕、分享、播放量和发布距今天数形成的互动质量倍率。
- `actual_compression_ratio`：笔记字数 / 字幕字数。它是信息密度参考，不是强制压缩目标；长课程可以保留更多细节。
...
- 写前先读取 `note_budget.json` 了解信息量和画面依赖，再开始写笔记；写后评分只用于验收和发现遗漏。没有最长限制，不因 `too_long` 自动删减内容，只处理确实重复或误收录的段落。
